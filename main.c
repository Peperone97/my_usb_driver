...
void transfer( struct libusb_device_handle *device_handle ){
	uint8_t msg[] = {  0x1c, 0x00, 0xc0, 0x97, 0x69, 0xa5, 0x89, 0xe5, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x1b, 0x00, //16
					   ...
					   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xfb, 0x00 //14
	};

	int i = 0;
	int read = 0;

	int err = libusb_interrupt_transfer( device_handle, 0x81, msg, sizeof(msg), &read, 8000 );
	if( err != 0 ){
...
void keyboardPrv( struct libusb_device_handle *device_handle ){
	...
	int err = libusb_bulk_transfer( device_handle, 0x82, str, 256, &read, 2000 );
				...
				if( desc->idVendor == razer_vendor_id ){
					...
					if( (err = libusb_open( devices[i], &dev_handle )) ){
						...
						if( desc->idProduct == keyboard ){
							//printf("Speed: %d\tMax pkg size: %d\n", libusb_get_device_speed( devices[i] ), libusb_get_max_packet_size( devices[i], 0x82 ) );
							//printf( "%d\n", libusb_get_device_speed( devices[i] ) );
